...
public class BufferPool {
    ...
    public  Page getPage(TransactionId tid, PageId pid, Permissions perm)
    	...
    	for (PageId id : cache.keySet()) {
    		Page candidate = cache.get(id);
    		if (candidate.getId().equals(pid)) {
    			return candidate;
    		}
    	}
    ...
    public  void deleteTuple(TransactionId tid, Tuple t)
    	...
    	ArrayList<Page> pagesModified = fileToModify.deleteTuple(tid, t);
    	for (Page page : pagesModified) {
    		page.markDirty(true, tid);
    	}
    }
    ...
    */
    ...
    private synchronized  void flushPage(PageId pid) throws IOException {
    	...
    	if (page.isDirty() != null) {
    		DbFile fileToWrite = Database.getCatalog().getDatabaseFile(page.getId().getTableId());
    		fileToWrite.writePage(page);
